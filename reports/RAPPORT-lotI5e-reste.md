# RAPPORT — lot I5e (chantier i18n) : 246 clés `ui` — reste : fragments et libellés courts

**Livré en `GAME_BUILD = 412` / `GAME_VERSION = 'Alpha 17.9'`.** `SAVE_VERSION` INCHANGÉ (31).

| | |
|---|---|
| Base | **411** (sous-lot précédent, même branche) |
| Delta | **+50 910 o** pour le patch seul |
| Clés | **246 × 3 langues = 738 chaînes**, dont **51 fragments** de concaténation |
| Code de rendu touché | **aucun** — une IIFE en fin de bloc 6 |

Avec I5a-d, **les 389 clés réelles de l'inventaire I5 sont fournies**. Il ne reste que les **6 clés purement typographiques** (`Nv. `, `max`, ` min · `, ` kWh / kW`, ` → `, `∞ `), dont la traduction serait l'identité — l'inventaire demande de les ignorer.
...
**972** clés littérales re-collectées, puis `hasOwnProperty` vérifié en en/es/de : **246 / 246 clés du
...
**T2 — rendu écran (de). PASS.** 5 panneaux ouverts : **aucune des 227 clés détectables du lot n'apparaît en français**, et **23 traductions allemandes** y sont relevées (« Max. Durchsatz », « an », « nie », « Alle »…). (19 des 246 clés se traduisent à l'identique — « Auto », « Volume », « Plutonium », « nominal », « optimal »…)**

**T3 — fragments, phrase complète. PASS (25 phrases).** Chaque phrase est **assemblée
...
## ⚠ Constat hors périmètre — 3 queues françaises restent dans des GABARITS

Le T3 les a fait sortir. Ce sont des **fragments de gabarit** (`TemplateElement`) **jamais passés à
`I18N.t`** : ils ne peuvent donc pas figurer dans l'inventaire I5, qui ne recense que les clés
`I18N.t`. Mesuré sur le bloc 7 patché — **3 chaînes distinctes, 4 sites** :

| ligne | fragment resté français | phrase rendue en allemand |
|---|---|---|
| 28231 | `" s si rien ne change"` | « 🔥 Presse (Insel 6) sammelt Wärme an — Überhitzung in ~42 **s si rien ne change** » |
| 24250 | `" : besoin de "` | toast de pose refusée |
| 25249 · 27865 | `" ne se construit que sur "` | toast d'exclusivité d'île |

**Non corrigé** : les envelopper est exactement le travail du lot **I3** (qui l'a fait pour 19 autres
quasis, sur inventaire explicite) — élargir unilatéralement ce lot-ci sortirait du périmètre. À
prendre dans un lot dédié ou avec I6.
