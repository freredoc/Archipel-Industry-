# RAPPORT — lot I5b (chantier i18n) : 23 clés `ui` — souterrain / forage / élévateur

**Livré en `GAME_BUILD = 409` / `GAME_VERSION = 'Alpha 17.6'`.** `SAVE_VERSION` INCHANGÉ (31).

| | |
|---|---|
| Base | **408** (sous-lot précédent, même branche) |
| Delta | **+7,998 o** pour le patch seul |
| Clés | **23 × 3 langues = 69 chaînes**, dont **4 fragments** de concaténation |
...
**972** clés littérales re-collectées, puis `hasOwnProperty` vérifié en en/es/de : **23 / 23 clés du
...
**T2 — rendu écran (de). PASS.** 6 surfaces réellement ouvertes, dont la **fiche de foreuse** posée puis ouverte par un **vrai tap canvas** sur l'île 7 : **aucune des 23 clés du lot n'apparaît en français**, et 4 traductions allemandes y sont relevées (« Ring (Aufzug) », « Grabrichtung », « Wand durchbrechen », « wartet auf den Aufzug »).**
...
⚠ **Piège de banc** : l'**inventaire ouvert recouvre le canvas** (14.89) — sans le replier, le tap
atterrit sur un `span.inv-item` et la fiche ne s'ouvre jamais. C'est ce qui faisait échouer
l'ouverture de la fiche de foreuse alors que la pose, elle, réussissait.
