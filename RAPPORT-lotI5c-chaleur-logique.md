# RAPPORT — lot I5c (chantier i18n) : 37 clés `ui` — chaleur / refroidissement / couche logique

**Livré en `GAME_BUILD = 410` / `GAME_VERSION = 'Alpha 17.7'`.** `SAVE_VERSION` INCHANGÉ (31).

| | |
|---|---|
| Base | **409** (sous-lot précédent, même branche) |
| Delta | **+12,882 o** pour le patch seul |
| Clés | **37 × 3 langues = 111 chaînes**, dont **8 fragments** de concaténation |
...
**972** clés littérales re-collectées, puis `hasOwnProperty` vérifié en en/es/de : **37 / 37 clés du
...
**T2 — rendu écran (de). PASS sur le critère central, partiel sur le second.** 6 surfaces ouvertes, dont la **fiche de refroidisseur** (posée sur l'île 6, ouverte par un vrai tap canvas) : **aucune des 37 clés du lot n'apparaît en français**. La fiche relevée est intégralement allemande (« KÜHLUNG », « nicht an eine Leitung angeschlossen », « kein Kühlmittel → 65,5 MJ/s aufgenommen »). En revanche **0 clé de ce sous-lot précis n'y était visible** : « Bilan chaleur », « Tampon de la cage », « Faces inhibées » n'apparaissent que si le bâtiment a effectivement de la chaleur, un conduit ou une porte logique câblée. Couverture assurée par T3 (8 phrases).**

**T3 — fragments, phrase complète. PASS (8 phrases).** Chaque phrase est **assemblée
...
⚠ **Piège de banc** : l'`innerText` des fiches est en **MAJUSCULES** (CSS `text-transform`,
mémo 14.61) — toute comparaison de libellé doit être **insensible à la casse**, sinon on conclut à
tort qu'une chaîne traduite est absente de l'écran (c'est arrivé, et corrigé).
